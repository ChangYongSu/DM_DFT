...
void DlgModelSetupSeq::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Control(pDX, IDC_EDIT_WIFI_MACADDRESS, m_EditCheckWifi);
}
...
//	ON_EVENT(DlgModelSetupSeq, IDC_BUTTON_IMAGE_SET, DISPID_CLICK, DlgModelSetupSeq::ClickButtonImageSet, VTS_NONE)
	...
	ON_EVENT(DlgModelSetupSeq, IDC_LABEL_CHECKSUM, DISPID_CLICK, DlgModelSetupSeq::ClickLabelChecksum, VTS_NONE)
	ON_EVENT(DlgModelSetupSeq, IDC_LABEL_CHECK_WIFI, DISPID_CLICK, DlgModelSetupSeq::ClickLabelCheckWifi, VTS_NONE)
END_EVENTSINK_MAP()
...
void DlgModelSetupSeq::DisplayTestData()
{
	...
	{
		strTmp.Format(_T("0x%02X 0x%02X"), gTestSeqData.CheckSum[0], gTestSeqData.CheckSum[1]);
	}
	else
	{
		strTmp.Format(_T("0x%02X 0x%02X 0x%02X 0x%02X"), gTestSeqData.CheckSum[0], gTestSeqData.CheckSum[1], gTestSeqData.CheckSum[2], gTestSeqData.CheckSum[3]);
		CString str;
		BYTE lbuf[5];
		lbuf[4] = 0;
	...
	SetDlgItemText(IDC_EDIT_CHECKSUM, strTmp);
...
int  DlgModelSetupSeq::UpdateTestData()//gTestSeqData
{
	...
	GetDlgItemText(IDC_EDIT_CHECKSUM, str1);//m_EditCheckSum;
	...
	if(m_RecvCntPCB > 2)
	{
		if(m_RecvBufPCB[0] == 0xBB)
		{
			...
			/*	for(int i = 0; i < m_RecvCntPCB; i++)
				...
				if (m_RecvBufPCB[3] == _COMMAND_CHECK_SUM_READ)
				{
					if (m_RecvCntPCB == 9)
					{
					...
					SetDlgItemText(IDC_EDIT_CHECKSUM, strReceive);
					
					//UpdateData(FALSE);
				}
				else if (m_RecvBufPCB[3] == _COMMAND_WIFI_MAC_READ)
				{
				...
				//	m_EditCheckWifi
					if (m_RecvCntPCB == 22)
					{
						...
						BYTE lbuf[20];
						lbuf[17] = 0;
						memcpy(lbuf, &(m_RecvBufPCB[4]),17);
						str = lbuf;
						SetDlgItemText(IDC_EDIT_WIFI_MACADDRESS, str); //SetDlgItemText(IDC_DISPLAY_CHECKSUM, str);
					}
					
					//strReceive = strtmp;

					//m_EditCheckSum = strReceive;s90ty s10ty
					//UpdateData(FALSE);
				}
...
void DlgModelSetupSeq::DblClickLabelChecksum(LPDISPATCH Cancel)
{
	...
	//int lType = m_cComboModelPcbType.GetCurSel();
	////CString Str;
...
void DlgModelSetupSeq::ClickLabelChecksum()
{
	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
	BYTE lbuf[128];
	int lcnt;
...
void DlgModelSetupSeq::ClickLabelCheckWifi()
{
	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
	...
	lbuf[3] = _COMMAND_WIFI_MAC_READ;//Wifi Mac Read
