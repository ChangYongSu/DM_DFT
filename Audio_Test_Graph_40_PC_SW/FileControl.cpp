...
extern CAcqVoltageSamples_IntClkDlg *mp_MainControl;
...
int CFileControl::CheckModelSeqFile(CString lFilename)
{
	...
	gTestSeqData.TestCnt = 0;
	gTestSeqData.PreTestCnt = 0;
	//mp_MainControl->m_BackGroundTestCnt = 0;
		...
		while (fgets(lbuf, 1024, m_fp) != NULL)
		{
			...
			if (lpos >= 0)
			{
				if (CheckStepLine(strTemp))
				{
					...
					str = lFilename + str;
					str += strTemp;
					MessageBox(mp_MainControl->m_hWnd, str, "Syntax Error", MB_OK);
					lFileError = 1;
					break;
				}
...
{
	int lError = mp_MainControl->SetFunction(StringLine, 1);
		...
		if(fgets(lbuf, 1024, m_fp) != NULL)
		{
			if(strncmp(lbuf,"[MAIN COM]",strlen("[MAIN COM]")) == 0)
			{
				...
				while(fgets(lbuf, 1024, m_fp) != NULL)
				{
					...
					else if (strncmp(lbuf, "[RUN TYPE]", strlen("[RUN TYPE]")) == 0)	//[NI CARD]%d\n", 					
					{
						sscanf_s(&lbuf[strlen("[RUN TYPE]")], "%d", &g_RunModeType);
					}
					else if (strncmp(lbuf, "[GMES AUTO CHANGE MODEL]", strlen("[GMES AUTO CHANGE MODEL]")) == 0)	//[NI CARD]%d\n", 					
					{
						sscanf_s(&lbuf[strlen("[GMES AUTO CHANGE MODEL]")], "%d", &g_MesAutoChangeModel);
					}
					else if (strncmp(lbuf, "[RPT_ID]", strlen("[RPT_ID]")) == 0)
					{
						lbuf[strlen(lbuf)] = 0;
						g_strRPTID = &lbuf[strlen("[RPT_ID]")];
						g_strRPTID.Trim();
					}
				}
...
int CFileControl::SaveConfigSeqData()//
{
	...
	if(m_fp)
	{
		...
		//fprintf(m_fp, "[SET COM]%d %d %d\n", gTestSeqData.nPort_Pcb, l_nBaud_Pcb, l_nParity_Pcb );
		...
		fprintf(m_fp, "[RUN TYPE]%d\n", g_RunModeType);
		fprintf(m_fp, "[GMES AUTO CHANGE MODEL]%d\n", g_MesAutoChangeModel);// sscanf_s(&lbuf[strlen("[GMES AUTO CHANGE MODEL]")], "%d", &g_MesAutoChangeModel);.GetString());
	//	fprintf(m_fp, "[PORT NUMBER]%d %d\n", g_nLocalPortNumber, g_nRemotePortNumber);
		fprintf(m_fp, "[RPT_ID]%s\n", g_strRPTID.GetString());
