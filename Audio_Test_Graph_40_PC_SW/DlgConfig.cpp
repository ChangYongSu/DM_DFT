...
void CDlgConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_AUTO_MODEL_CHANGE, m_cCheckAutoChangeModel);
	DDX_Control(pDX, IDC_COMBO_EPT_ID, m_cComboRPTID);
}
...
BOOL CDlgConfig::OnInitDialog()
{
	...
	m_nBaud = g_nBaud_Set;
	m_nPort = g_nPort_Set;
	m_nParity = g_nParity_Set;

	m_nBaudLED = g_nBaud_LED;
	m_nPortLED = g_nPort_LED;
	m_nParityLED = g_nParity_LED;

	m_nPortScan = g_nPort_Scanner;
	m_nBaudScan = g_nBaud_Scanner;
	m_nParityScan = g_nParity_Scanner;

	m_IPaddressGmes = g_nRemoteIPAddress;
	m_ServerPortNumberCM45.Format(_T("%d"), g_nRemotePortNumberCM45);
	m_ServerPortNumberCM89.Format(_T("%d"), g_nRemotePortNumberCM89);
	m_GmesWaitTime.Format(_T("%d"), g_nGmesWaitTime);
	m_nNI_CardSel = g_nNI_CardSel;
	m_cCheckAutoChangeModel.SetCheck(g_MesAutoChangeModel);
...
//	m_EditLEDPortString.Format(_T("%d"),m_nPortLED );
	...
	m_cComboRPTID.ResetContent();
	m_cComboRPTID.AddString("20100");
	m_cComboRPTID.AddString("31100");
	if (g_strRPTID == "31100")
	{
		m_cComboRPTID.SetCurSel(1);
	}
	else
	{
		m_cComboRPTID.SetCurSel(0);
	}
...
void CDlgConfig::OnBnClickedOk()
{
	...
	g_nBaud_Set = m_nBaud;
	g_nPort_Set = m_nPort;
	g_nParity_Set = m_nParity;



	g_nBaud_LED = m_nBaudLED;
	g_nPort_LED = m_nPortLED;
	g_nParity_LED = m_nParityLED;

	g_nPort_Scanner = m_nPortScan;
	g_nBaud_Scanner = m_nBaudScan;
	g_nParity_Scanner = m_nParityScan;

	g_nRemoteIPAddress = m_IPaddressGmes;
	//g_nLocalPortNumber = _ttoi(dlg.m_PortNumber);
	g_nRemotePortNumberCM45 = _ttoi(m_ServerPortNumberCM45);
	g_nRemotePortNumberCM89 = _ttoi(m_ServerPortNumberCM89);
	g_nGmesWaitTime = _ttoi(m_GmesWaitTime);
	...
	g_nNI_CardSel = m_nNI_CardSel;

	g_MesAutoChangeModel = m_cCheckAutoChangeModel.GetCheck();
	

	if (m_cComboRPTID.GetCurSel() == 1)
	{
		g_strRPTID = "31100";
	}
	else
...
void CDlgConfig::GetDeviceInfo()
{
	...
	DWORD i = 0;
	CString strTemp;
	memset(nListNumberPort, 0, sizeof(nListNumberPort));
	...
	//	wchar_t  szFriendly[MAX_PATH] = {0};
		...
		if (str1.Find("(COM") >= 0)
		{
			//UINT		nListNumberPort[100];
			//CString		sListNumberPort[100];
			strTemp = str1.Mid(str1.Find("(COM") + 4);
			strTemp = strTemp.Left(strTemp.Find(")"));
			...
			int lnPortID = _ttoi(strTemp);
			if (lnPortID < 100)
			{
				nListNumberPort[lnPortID] = 1;
				sListNumberPort[lnPortID] = str1;
			}
	...
	for (int i = 1; i < 100; i++)
	{
		if (nListNumberPort[i] == 1)
		{

			nNumberPort[m_PortCnt] = i;
			strPort[m_PortCnt] = sListNumberPort[i];
			m_PortCnt++;
		}
