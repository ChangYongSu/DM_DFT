...
//	DDX_Control(pDX, IDC_COMBO_COMPORT_LED, m_cComboLEDPort);
	...
	DDX_Check(pDX, IDC_CHECK_USE_USB_SCANNER, m_CheckUseUSB_Scanner);
...
BEGIN_MESSAGE_MAP(CDlgConfig, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_USE_USB_SCANNER, &CDlgConfig::OnBnClickedCheckUseUsbScanner)
...
void CDlgConfig::OnBnClickedOk()
{
	...
	else
	{
		...
		m_cComboPCBPort.GetLBText(m_cComboPCBPort.GetCurSel(), str1);
		l_port = _ttoi(str1.Mid(3));
		m_nPortPcb = l_port;
	...
	if((l_nNI_CardSel >= 0)&&(l_nNI_CardSel <= 3))
	{
...
//	m_cComboLEDPort.ResetContent();
	...
	if(m_PortCnt > 0)
	{
		memset(nNumberPort, 0, sizeof(nNumberPort));
		...
		for (int i = 1; i < 50; i++)
		{
			if (nNumberPort[i] == 1)
			{
				...
				if (m_nPortPcb == i)
				{
					m_cComboPCBPort.SetCurSel(lPortCount);
				}
				//if(m_nPortLED == nNumberPort[i])
				//{
				//	m_cComboLEDPort.SetCurSel(i);
...
void CDlgConfig::OnBnClickedCheckUseUsbScanner()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData();
	if(m_CheckUseUSB_Scanner == 0)
	{
		GetDlgItem(IDC_BUTTON_SCANNER_PORT_CHECK)->ShowWindow(1);
		GetDlgItem(IDC_COMBO_COMPORT_SCAN )->ShowWindow(1);
		GetDlgItem(IDC_COMBO_BAUD4		  )->ShowWindow(1);
		GetDlgItem(IDC_COMBO_PARITY4	  )->ShowWindow(1);
	}
	else
	{
		GetDlgItem(IDC_BUTTON_SCANNER_PORT_CHECK)->ShowWindow(0);
		GetDlgItem(IDC_COMBO_COMPORT_SCAN )->ShowWindow(0);
		GetDlgItem(IDC_COMBO_BAUD4		  )->ShowWindow(0);
		GetDlgItem(IDC_COMBO_PARITY4	  )->ShowWindow(0);
