...
extern CAcqVoltageSamples_IntClkDlg* m_pMainDlg;// CDATsysView*  g_pView;
...
_MeasureFunction MeasDefined[] =
{
	...
	_T("sm_mio_check"),	_SM_MIO_Check,		MEAS_BOOL,
...
void MakeFunctionList(CStep* pStep, char* fun)
{
	...
	{                             
		if (!strcmp(Predefined[nIndex].pszFunc, _strlwr(fun)))
        {
			...
			pFunction->m_strFuncName = Predefined[nIndex].pszFunc;
			CString str;
...
BOOL MakeSubStep(CStep* pStep, CStringArray& aArray)
{
	...
	{
		//+add kwmoon 080213
		if(m_pMainDlg->m_bAdjSpecMode && !pStep->m_bTest)
		{
