...
BOOL _NotifyMessage()
{
	...
	pCurFunc->m_nFuncTestType = _FUNC_WAIT_MANUAL_WAIT_TYPE;
	pCurFunc->m_nFuncTimeLimit = 100000;
	pos = pCurFunc->m_ArgumentList.GetHeadPosition();
	if (pos!= NULL)
	{
        sMsg1 = GetString();
		...
		CRect rectP;
		CRect rect;	
		g_pView->GetWindowRect(&rectP);
		g_pView->m_pUserMsgDlg->GetWindowRect(&rect);

		//g_pView->m_pUserMsgDlg->ShowWindow(SW_SHOW);


		g_pView->m_pUserMsgDlg->SetMessage(sMsg1,sMsg2);
		g_pView->m_pUserMsgDlg->ShowWindow(SW_SHOW);
		g_pView->m_pUserMsgDlg->SetWindowPos(g_pView,rectP.left+ 30,rectP.top+350,rect.Width(),rect.Height(),SWP_ASYNCWINDOWPOS);
...
BOOL _Delay()
{
	...
	pCurFunc->m_nFuncTestType = _FUNC_WAIT_DELAY_TYPE;
	//if (((double)(clock() - sFuncStepStart) / CLOCKS_PER_SEC) > pCurFunc->m_nFuncTimeLimit)
	...
	pCurFunc->m_nFuncTimeLimit = nDelay;
	//if(!_Wait(nDelay)) return FALSE;
	return TRUE;
}
...
BOOL Work_Normal()
{
	...
	pCurFunc->m_nFuncTestType = _FUNC_WORK_TEST_TYPE;
	//+chage kwmoon 080402
	double reval = 0;
	...
	DWORD dwExitCode = 0;
	pCurFunc->m_nFuncTimeLimit = (g_pView->m_nMeasureWait > 0) ? g_pView->m_nMeasureWait : 1000;
	return TEST_PASS;
...
BOOL Work_Normal_Check()
{
	...
	if (pCurStep->m_bRunAudioTest == TRUE)
	{
		...
		{
			if (pCurStep->m_bAudioTestResult == TRUE)
			{
			}
			else
			{
		...
		//sTemp.Format("Audio:F %d/%d, L:%d/%d\r\nF %d/%d, L:%d/%d\n",
		//	pCurStep->m_nLeftFreq[0], pCurStep->m_nLeftFreq[1],
		//	pCurStep->m_nLeftLevel[0], pCurStep->m_nLeftLevel[1],
		//	pCurStep->m_nRightFreq[0], pCurStep->m_nRightFreq[1],
		//	pCurStep->m_nRightLevel[0], pCurStep->m_nRightLevel[1]);
		//g_pView->ComLog_AddString(sTemp);
	}
...
BOOL _Remote_Out()
{
	...
	//	}
	//	if(!_Wait(nDelay)) return FALSE;
	pCurFunc->m_nFuncTestType = _FUNC_AV_SWITCH_CTRL_TYPE;
	AvSwitchBoxCtrl.m_sRemoteCode = sCmd;
	AvSwitchBoxCtrl.m_nRemoteDelay = nDelay;
	AvSwitchBoxCtrl.m_nNoRemoteRetry = nClickNum;
	AvSwitchBoxCtrl.m_nNoRemoteRetryCount = 0;
	//AvSwitchBoxCtrl.SendRemoteCmd(MAX_REMOTE_WAIT_DELAY, CurrentSet->nRemoteType, sCustom, sCmd, 1);
	lResult = AvSwitchBoxCtrl.SendRemoteCmd(MAX_REMOTE_WAIT_DELAY, CurrentSet->nRemoteType, sCustom, sCmd, 1);
	return lResult;	//_Wait(nDelay);
...
BOOL _SetTVCommand()
{
	...
	pCurFunc->m_nFuncTestType = _FUNC_UART_COMMAND_TEST_TYPE;
...
{
	BOOL bResult = FALSE;
	pCurFunc->m_nFuncTestType = _FUNC_VIDEO_TEST_TYPE;
...
//	sGrabPath.Format("%s\\S%d_%s",CurrentSet->sGrabFolder, pCurStep->m_nStep, sRefTmp);
	...
	pCurStep->m_strMaskPath= CurrentSet->sMaskFolder + "\\" + sRefTmp;;
...
//	nGrabResult			= GRAB_FAIL;
	...
	//							  ptResultPosition,fMaxDifference,nTotalNoErrorPixel,nUseMaskImage);
	...
	if(nResult == TEST_ABORT)
	{
		if(CurrentSet->bSaveGrabImg)
		{
			g_GrabImage.SaveImage(pCurStep->m_strGrabPath, SNAP_IMAGE);
		}
		g_ImageProc.SetRoi(ptZeroPosition,ptZeroPosition);
		return TEST_ABORT;
	}
...
{
	return Grab_Image_Check_Normal_2in1();
//	int nMaxDelay		 = 0;
//	int nUseMaskImage	 = 0;	// 0 : Dont't Use, 1 : Use
//	int nRetry = 0;
//	CString sRefTmp					= _T("");
...
BOOL Grab_Image_2in1(CString sRefImagePath, int nUseMaskImage, BOOL bMultiGrab)
{
	...
	DWORD dwFuncElapsedTime = 0;
	CString szFuncElapsedTime = _T("");
	g_pView->ScreenCapture(pCurStep->m_strGrabPath, "");
//	if(!RefImage_DirectLoad(sRefImagePath, nUseMaskImage)) return TEST_ABORT;
	//g_pView->m_pImageData

	nWidth = g_pView->m_ImageWidth;
	nHeight = g_pView->m_ImageHeight;
	nImageSize = g_pView->m_ImageSize;
	...
	nGrabResult = GRAB_PASS;////nGrabResult = AnalogControl.OnGrabStart(GRAB_WITH_RETRY, sRefImagePath, nUseMaskImage, bMultiGrab);
	...
	if (nGrabResult == GRAB_PASS)
	{
		...
		//	{
		//		g_GrabImage.m_pImageData[i*nRowWidth + 3 * j + 2] = (BYTE)(g_pView->m_pImageData[(i)*nRowWidth + 3 * j + 0]);  // R
		//		g_GrabImage.m_pImageData[i*nRowWidth + 3 * j + 1] = (BYTE)(g_pView->m_pImageData[(i)*nRowWidth + 3 * j + 1]); // G
		//		g_GrabImage.m_pImageData[i*nRowWidth + 3 * j] = (BYTE)(g_pView->m_pImageData[(i)*nRowWidth + 3 * j+2]); // B
		//	}
		//}
	}
	else
...
BOOL _CheckAudio()
{
	...
	int nAudioSource = 0;
	float l_bMeasureAudioType = FREQ_LEVEL_CHECK;
	float l_fTargetLeftFreq = 0;
	float l_fTargetRightFreq = 0;
	float l_fTargetLeftLevel = 0;
	float l_fTargetRightLevel = 0;
	float l_fFreqMargin = 0;
	float l_fLevelMargin = 0;
...
//	g_pView->m_bMeasureAudio = FALSE;
	...
	for (int i = 0; i < 8; i++)
	{
		...
		g_pView->m_RefVoltageMin[i] = 0;
		g_pView->m_RefVoltageMax[i] = 0;
		g_pView->m_FrequencyMin[i] = 0;
		g_pView->m_FrequencyMax[i] = 0;
	}
	if (pos!= NULL)
	{
		nAudioSource	= GetInteger();

		l_bMeasureAudioType = FREQ_LEVEL_CHECK;
		l_fTargetLeftFreq	= (float)GetInteger();
		l_fTargetRightFreq	= (float)GetInteger();
		l_fTargetLeftLevel	= (float)GetInteger();
		l_fTargetRightLevel	= (float)GetInteger();
		l_fFreqMargin		= (float)(float)GetInteger();		
		l_fLevelMargin		= (float)(float)GetInteger();

		
		g_pView->m_RefVoltageMin[0] = l_fTargetLeftLevel - l_fLevelMargin;
		g_pView->m_RefVoltageMax[0] = l_fTargetLeftLevel + l_fLevelMargin;
		g_pView->m_RefVoltageMin[1] = l_fTargetRightLevel - l_fLevelMargin;
		g_pView->m_RefVoltageMax[1] = l_fTargetRightLevel + l_fLevelMargin;
		g_pView->m_FrequencyMin[0] = l_fTargetLeftFreq - l_fFreqMargin;
		g_pView->m_FrequencyMax[0] = l_fTargetLeftFreq + l_fFreqMargin;
		g_pView->m_FrequencyMin[1] = l_fTargetRightFreq - l_fFreqMargin;
		g_pView->m_FrequencyMax[1] = l_fTargetRightFreq + l_fFreqMargin;
...
BOOL RefImage_DirectLoad(CString szFileName,int nUseMaskImage)
{
	...
	if(!g_RefImage.LoadImage(sTmp)) return FALSE;
	...
	if(nUseMaskImage)
	{
		...
		if(!g_MaskImage.LoadImage(sTmp)) return FALSE;
	}
	return TRUE;
}
...
BOOL _SetRelay_OnOff()
{
	...
	bRet = gSMDIO_Ctrl.SetTargetDo(nRelayNo - 1, nFlag);
...
BOOL _MIC_Set()
{
	...
	if (nChannelNo == 1)
	{
		g_pView->SetFreqSend(12, nFlag); 
		bRet = 1;
		g_pView->m_MIC_Status1 = nFlag;
	}
	else if(nChannelNo == 2)
	{
		g_pView->SetFreqSend(13, nFlag);
		bRet = 1;
		g_pView->m_MIC_Status2 = nFlag;
	}
	else
	{
		bRet = 0;
