...
CAcqVoltageSamples_IntClkDlg* g_pView;
...
BOOL CAcqVoltageSamples_IntClkApp::InitInstance()
{
	...
	{
#ifdef	_DEBUG
		lfname = _T("DM_HDFT1_D.exe");//HomeViewAutoTester.exe
#else
		lfname = _T("DM_HDFT1.exe");
#endif
	}
	else
	{
#ifdef	_DEBUG
		lfname = _T("DM_HDFT2_D.exe");//HomeViewAutoTester.exe
#else
		lfname = _T("DM_HDFT2.exe");
#endif
	}
	if( GetProcessList(lfname, 1024, 0) > 1)
	{
		CString strTmp;
		strTmp = lfname;
		strTmp += " Program Running Detected!!\r\n\r\nTerminate Old Program??";
