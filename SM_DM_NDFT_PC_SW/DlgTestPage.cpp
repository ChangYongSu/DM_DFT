...
void CDlgTestPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_FREQ1, m_EditFreq[0]);
	...
	DDX_Control(pDX, IDC_COMBO_RM_TEST1, m_cComboRmTest);
	...
	DDX_Control(pDX, IDC_COMBO_VOLT_NO1, m_cComboVoltNo[0]);
	DDX_Control(pDX, IDC_COMBO_VOLT_NO2, m_cComboVoltNo[1]);
	DDX_Control(pDX, IDC_COMBO_VOLT_NO3, m_cComboVoltNo[2]);
	DDX_Control(pDX, IDC_SLIDER_FREQ, m_cSliderFrequency);
	DDX_Text(pDX, IDC_EDIT_FREQ_SLIDE, m_EditSlideData);
	DDX_Control(pDX, IDC_CHECK_USB1, m_CheckUSB1);
	DDX_Control(pDX, IDC_CHECK_USB2, m_CheckUSB2);
	DDX_Control(pDX, IDC_CHECK_MIC1, m_cCheckMic1);
	DDX_Control(pDX, IDC_CHECK_MIC2, m_cCheckMic2);
}
...
BEGIN_MESSAGE_MAP(CDlgTestPage, CDialogEx)
	...
	//ON_BN_CLICKED(IDC_BUTTON_FREQ11, &CDlgTestPage::OnBnClickedButtonFreq11)
	ON_BN_CLICKED(IDC_BUTTON_FREQ12, &CDlgTestPage::OnBnClickedButtonFreq12)

	ON_BN_CLICKED(IDC_BUTTON_FREQ_SET_TEST, &CDlgTestPage::OnBnClickedButtonFreqSet)
...
//	ON_BN_CLICKED(IDC_BUTTON_UPDIN_READ, &CDlgTestPage::OnBnClickedButtonUpdinRead)
	ON_BN_CLICKED(IDC_BUTTON_VIB_SWEEP, &CDlgTestPage::OnBnClickedButtonVibSweep)
	ON_WM_HSCROLL()
	ON_BN_CLICKED(IDC_BUTTON_SLIDE_SET, &CDlgTestPage::OnBnClickedButtonSlideSet)
	ON_BN_CLICKED(IDC_BUTTON_UPDIN_READ, &CDlgTestPage::OnBnClickedButtonUpdinRead)
	
	ON_BN_CLICKED(IDC_CHECK_USB1, &CDlgTestPage::OnBnClickedCheckUsb1)
	ON_BN_CLICKED(IDC_CHECK_USB2, &CDlgTestPage::OnBnClickedCheckUsb2)
	ON_BN_CLICKED(IDC_BUTTON_SAVE_FREQ, &CDlgTestPage::OnBnClickedButtonSaveFreq)
	ON_BN_CLICKED(IDC_BUTTON_FREQ13, &CDlgTestPage::OnBnClickedButtonFreq13)
	ON_BN_CLICKED(IDC_CHECK_MIC1, &CDlgTestPage::OnBnClickedCheckMic1)
	ON_BN_CLICKED(IDC_CHECK_MIC2, &CDlgTestPage::OnBnClickedCheckMic2)
END_MESSAGE_MAP()
...
BOOL CDlgTestPage::OnInitDialog()
{
 ...
 //   // Return TRUE unless you set the focus to a control.
	...
	for(int i = 0; i < 3; i++)
	{
		for(int j = 0; j < 18; j++)
		{
			sTmp.Format("CH %d", j+1);
			m_cComboVoltNo[i].AddString(sTmp);
		}		
	}
	m_cComboVoltNo[0].SetCurSel(0);
	m_cComboVoltNo[1].SetCurSel(1);
	m_cComboVoltNo[2].SetCurSel(2);
	...
	m_pMainDlg->SetUSBSel(USB_SEL_UART_MODE_, USB_SEL_UART_MODE_);
	m_WaitPCB_Ack_Limit = 60;
	if (CurrentSet->nRemoteType != 1)
	{
		CurrentSet->nRemoteType = 0;
	}
	//if (CurrentSet->nRemoteType == 1)
	//{
	//	gTestSeqData.nRemoconSigType = 0;
	//}
	//else
	//{
	//	gTestSeqData.nRemoconSigType = 1;
	//}
	m_cCheckMic1.SetCheck(m_pMainDlg->m_MIC_Status1);
	m_cCheckMic2.SetCheck(m_pMainDlg->m_MIC_Status2);
...
BOOL CDlgTestPage::InitPage()
{
	...
	*/
	...
	m_nPortPcb = _ttoi(CurrentSet->sTVComPort.Mid(CurrentSet->sTVComPort.Find("COM") + 3));
	m_nBaudPcb = CurrentSet->wTVBaudRate;
	m_nParityPcb = CurrentSet->nTVParity;
	...
	if (cfgdlg.m_PortCnt > 0)
	{
		for (int i = 0; i < cfgdlg.m_PortCnt; i++)
		{
			...
			strTemp = cfgdlg.strPort[i].Mid(cfgdlg.strPort[i].Find("(COM") + 4);
			strTemp = strTemp.Left(strTemp.Find(")"));

			strTempCom = cfgdlg.strPort[i].Mid(cfgdlg.strPort[i].Find("(COM") + 1);
		...
		for (int i = 1; i < 50; i++)
		{
			if (nNumberPort[i] == 1)
			{
				...
				{
					m_cComboPCBTempPort.SetCurSel(lPortCount);
				}
				lPortCount++;
			}
		}
	...
	//	}
	//
	////	strTemp.Format(_T("哪器飘 %d俺啊 八祸登菌嚼聪促."), m_cComboPortSel.GetCount());
	////	MessageBox(strTemp);		
	//			
	//}
	//else
	...
	for(int i = 0; i < 5; i++)
	{
		if(CurrentSet->wTVBaudRate == mbaud_List[i])
		{
	...
	m_cComboTempSetParity.AddString("MARK");
	m_cComboTempSetParity.AddString("SPACE");
	m_cComboTempSetParity.SetCurSel( CurrentSet->nTVParity);



	GetRemoconList();

	GetRmtCodeList(CurrentSet->sRemoconTitle);//GetRmtCodeList(CString lRmtFile)//gTestSeqData.strRemoconFileName;

	m_cComboSignalCarrier.AddString("CABLE WIRE");
	m_cComboSignalCarrier.AddString("IR REMOTE");
	m_cComboSignalCarrier.SetCurSel(CurrentSet->nRemoteType);
...
//	int m_nRecomonCount = 0;
	...
	CString strWildcard;
		...
		//	PathNameList[nFileCnt] = pathName;
			fileNameList[nFileCnt] = pathName;// fileName;
		...
		if(bFileFound)
		{
			...
			*/
			...
			for(int i = 0 ; i < nFileCnt; i++)
			{
				...
				if(m_strRemoconList[i].Compare(CurrentSet->sRemoconTitle) == 0)
				{
					g_nRemoconSelNum = i;
				}
			}
...
BOOL CDlgTestPage::GetRmtCodeList(CString lRmtFile)//gTestSeqData.strRemoconFileName;
{
...
// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
	...
	if(m_fp)
	{
		...
		m_cComboRmTest.ResetContent();
		for (int i = 0; i < m_RemoconModel.m_Cnt; i++)
		{
			str = m_RemoconModel.Data[i].m_Company;
			str += _T(" : ");
			str += m_RemoconModel.Data[i].m_Name;
			m_cComboRmTest.AddString(str);
		}

		m_cComboRmTest.SetCurSel(0);
		
		m_pMainDlg->m_RemoconModel = m_RemoconModel;
		return 1;
	} 
	else
	{
...
void CDlgTestPage::OnTimer(UINT_PTR nIDEvent)
{
	...
	case COM_MODE_WAIT_ACK:
		...
		if(g_pView->m_ComAck == COM_ACK_OK)
		{
		...
		else if(g_pView->m_ComAck == COM_ACK_BUSY)
		{
			//MessageBox("COM Fail !! ");
...
//		m_ComPort.ClosePort();
//	}
//	if(m_ComPort.InitPort(this, _ttoi(CurrentSet->sAvSwitchBoxComPort.Mid(CurrentSet->sAvSwitchBoxComPort.Find("COM"+3))), CurrentSet->wAvSwitchBoxBaudRate, 'N', 8, 1, EV_RXCHAR, 4096 ))
//	{		
//		m_ComPort.StartMonitoring();
//	}
//	else
//	{
//		str.Format(_T("Set COM Port %d Open FAIL !!"), CurrentSet->wAvSwitchBoxBaudRate);
//		//MessageBox(str);
...
//	else
//	{
//		str.Format(_T("Set COM Port %d Open FAIL !!"), g_nPort_Scanner, g_nBaud_Scanner);
//		//MessageBox(str);
//		MessageDisplay(1, str);
//		/*StatusDisplayMsg(str, 1 );*/
//	}
	...
	if(m_RecvCntPCB > 2)
	{
		if(m_RecvBufPCB[0] == 0xBB)
		{
			...
			//	memset(lbuf,0,sizeof(lbuf));
				...
				for (int i = 0; i < m_RecvCntPCB; i++)
				{
					if((i >= 4)&&(i < m_RecvCntPCB-1) && ((m_RecvBufPCB[i] >= 0x30) && (m_RecvBufPCB[i] <= 'z'))|| (m_RecvBufPCB[i] == ' '))
					{
						if (lAscii == 0)
						{
							strtmp = " \"";
				...
				SetDlgItemText(IDC_RXDATA_DISPLAY,strReceive.GetBuffer());
				m_PCBComAck = COM_ACK_OK;
				m_RecvCntPCB = 0;
...
//	return 0;
//}
//void	CDlgTestPage::SendData(BYTE *lbuf, int lcnt, int lComID_Port)
//{
//	
//	//02 M nn [dd:"TransData dd Byte"] 03 0d 0a
...
////	SendData(lbuf, 21, COM_ID_AUDIO_PORT);// ComSend(lbuf, 21);
////
...
void CDlgTestPage::OnBnClickedButtonFreq12()
{
	// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
...
void CDlgTestPage::OnBnClickedButtonFreq13()
{
	// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
...
{
	CString sCmd;
	m_cComboRmTest.GetLBText(lRmtCodeIndex,sCmd);
	sCmd = sCmd.Mid(sCmd.Find(":") + 1);
	sCmd.Trim();
	if (m_pMainDlg->SendIR_Code(sCmd, 0))
	{
...
{
	// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
	int lRmtCodeIndex = m_cComboRmTest.GetCurSel();
...
//				//g_nPort_Pcb = dlg.m_nPortPcb  ;
//
//				if(	gTestSeqData.nPort_Pcb != m_nPortPcb)
//				{
//					m_nPortPcb = gTestSeqData.nPort_Pcb;
//					for(int i = 0; i < m_cComboPCBTempPort.GetCount(); i++)
//					{
...
void CDlgTestPage::OnBnClickedButtonMacSet()
{
	...
	lbuf[14] = 0x0a;
	//ReceiveData[18] == 0x03) && (ReceiveData[19] == 0x0d) && (ReceiveData[20] == 0x0a
	g_pView->SendData(lbuf, 15, COM_ID_AUDIO_PORT);//ComSend(lbuf, 15);
...
//{
//	// TODO: 
//	if (m_pMainDlg->m_MIC_Status1 == 1)
//	{
//		m_pMainDlg->SetFreqSend(10, 0);
//		m_pMainDlg->m_MIC_Status1 = 0;
//	}
//	else
//	{
//		m_pMainDlg->SetFreqSend(10, 1);
//		m_pMainDlg->m_MIC_Status1 = 1;
//	}
//		
//}
//
...
//	if (m_pMainDlg->m_MIC_Status1 == 1)
//	{
//		m_pMainDlg->SetFreqSend(11, 0);
//		m_pMainDlg->m_MIC_Status2 = 0;
//	}
//	else
//	{
//		m_pMainDlg->SetFreqSend(11, 1);
//		m_pMainDlg->m_MIC_Status2 = 1;
//	}
//}
...
void CDlgTestPage::OnCbnSelchangeComboComportTempPcb()
{
	...
	CString str1;
	int l_port;
	m_cComboPCBTempPort.GetLBText(m_cComboPCBTempPort.GetCurSel(), str1);
...
void CDlgTestPage::OnBnClickedButtonVibSweep()
{
	//// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
	//BYTE lbuf[128];
	//int lcnt;
	...
	////sprintf((char *)&lbuf[1], "CFREQ%02d%03d", 
	//sprintf((char *)&lbuf[1], "SFREQ%02dVIB       ", lPort);
	...
	//MessageDisplay(2, "VIB Set !");
}


void CDlgTestPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	//// TODO: 咯扁俊 皋矫瘤 贸府扁 内靛甫 眠啊 棺/肚绰 扁夯蔼阑 龋免钦聪促.
	//// m_cSliderFrequency
	//CString sTEmp;
	////static int sCntDisplay = 0;
	//static int s_SliderFreqVal = 0;
	//if (pScrollBar->GetDlgCtrlID() == IDC_SLIDER_FREQ)
	//{
	//	//	UpdateData(TRUE);
	//	m_SliderFreqVal = m_cSliderFrequency.GetPos();
	//	if (s_SliderFreqVal != m_SliderFreqVal)
	//	{
	//		//DisplaySaveData(m_SliderReview);
	//		m_EditSlideData.Format(_T("%d"), m_SliderFreqVal);
	//		UpdateData(FALSE);		
	//		SetFreqSend(4, m_SliderFreqVal);
	//	}
	//	s_SliderFreqVal = m_SliderFreqVal;
...
void CDlgTestPage::OnBnClickedButtonSlideSet()
{
	// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
//	UpdateData();
//	SetFreqSend(4, m_SliderFreqVal);
...
void CDlgTestPage::OnBnClickedCheckUsb1()
{
	// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.

	if (m_CheckUSB1.GetCheck())
	{
		m_pMainDlg->SetUSBSel(3, USB_SEL_PLAY_MODE_);
	}
	else
	{
		m_pMainDlg->SetUSBSel(3, USB_SEL_UART_MODE_);
	}
	
}


void CDlgTestPage::OnBnClickedCheckUsb2()
{
	// TODO: 咯扁俊 牧飘费 舅覆 贸府扁 内靛甫 眠啊钦聪促.
	if (m_CheckUSB2.GetCheck())
	{
		m_pMainDlg->SetUSBSel(0, 3);
	}
	else
	{
		m_pMainDlg->SetUSBSel(1, 3);
	}
}


void CDlgTestPage::OnBnClickedButtonSaveFreq()
{
	// TODO: 
...
void CDlgTestPage::OnBnClickedCheckMic1()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pMainDlg->m_MIC_Status1 = m_cCheckMic1.GetCheck();
	
	m_pMainDlg->SetFreqSend(12, m_pMainDlg->m_MIC_Status1);
	

}


void CDlgTestPage::OnBnClickedCheckMic2()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pMainDlg->m_MIC_Status2 = m_cCheckMic2.GetCheck();
	m_pMainDlg->SetFreqSend(13, m_pMainDlg->m_MIC_Status2);
}
