...
//
//-----------------------------------------------------------------------------
int CMixerBase::Init( DWORD DstType, DWORD SrcType, DWORD srcIndex, DWORD ControlType, HWND hwnd )
{	
	...
	{
		if(  GetSourceLineIndex( line, SrcType, srcIndex) == -1 )
		{
			PrintMessage( "CMixerBase::Init",
	...
	MMRESULT result;
	result = mixerGetLineControls( ( HMIXEROBJ )m_HMixer, &mixerLineControl, 
...
// Arg    : DWORD lineType  : 
//-----------------------------------------------------------------------------
int CMixerBase::GetSourceLineIndex( MIXERLINE &line, DWORD lineType, DWORD srcIndex)
{
	...
	for( UINT j = 0; j < nconn; j++ )
	{
		...
		if( line.dwComponentType == lineType &&  line.dwSource == srcIndex)
		{
			return( j );
		}			
	}
