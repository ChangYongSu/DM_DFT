...
BEGIN_MESSAGE_MAP(CDlgSimpleInform, CDialogEx)
	...
	ON_BN_CLICKED(IDNG, &CDlgSimpleInform::OnBnClickedNg)
END_MESSAGE_MAP()
...
//	m_Close_Result =  _OPEN_DISPLAY_;
	...
	if(m_WaitMassage == 1)
	{
		GetDlgItem(IDNG)->EnableWindow(TRUE);
	}
	else
	{
		GetDlgItem(IDNG)->EnableWindow(FALSE);
	}
	if(bShow)
		SetTimer(1, 100, NULL);
...
void CDlgSimpleInform::OnBnClickedNg()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_Close_Result =  _CLOSE_DISPLAY_NGCLICK;
	CDialogEx::OnOK();
}
